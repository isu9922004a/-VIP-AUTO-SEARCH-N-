...
function anchoredVolumeProfileEvidence(report){
  ...
  if(rows.length<SHITO_ASSISTANT_RULES_V1.minDailyBars)return {status:"unavailable",label:"日K資料不足20根，先不計算",anchorType:anchor.type,anchorStartDate:rows[0]?.date||anchor.date,anchorEndDate:rows.at(-1)?.date||report?.closeDate,dataDate:rows.at(-1)?.date||report?.closeDate,volumeProfileDataQuality:"資料不足",calculationMethod:"48個價格箱；每根日K以（最高＋最低＋收盤）÷3代表",rowsUsed:rows.length};
  ...
  return {status:"pass",label:"已用盤後日K完成區段估算",anchorType:anchor.type,anchorStartDate:rows[0].date,anchorEndDate:rows.at(-1).date,pocPrice:(poc.lower+poc.upper)/2,pocLower:poc.lower,pocUpper:poc.upper,pocVolumeShare:total?poc.volume/total*100:null,nearestHvnZone:nearH,nearestLvnZone:nearL,currentPriceVsPoc:close>poc.upper?"above":close<poc.lower?"below":"inside",pocRole:role,pocRetestState:held?(confirmations>=3&&executionAllowed?"拉回後守穩，輔助證據足夠":"拉回有守，但原有進場檢查尚未允許，不能當買進訊號"):touched?"最近碰到此區，尚未完成守穩確認":"最近10個交易日尚未回測此區",supportConfirmations:confirmations,volumeProfileDataQuality:`有效日K ${rows.length} 根`,calculationMethod:"價格分成48箱；每根日K以（最高＋最低＋收盤）÷3歸入一箱並累加成交量，屬區段估算，不是逐筆成交分布",dataDate:rows.at(-1).date,rowsUsed:rows.length,boxWidth:width};
}
...
function buildShitoAssistantEvidence(report,raw){
  ...
  return {version:"石頭少爺 Agent V47 正式版｜R5.3.2.4.4 封板驗收修正版",operatingGateEvidence:operating,holderGateEvidence:holder,capitalGateEvidence:capital,anchoredVolumeProfileEvidence:poc,momentumBreakoutEvidence:momentum,summary:`通過${main.pass}／${main.total}重${main.missing?`｜另有${main.missing}重資料不足`:""}`,strengths:strengths.slice(0,2),risks:risks.slice(0,2),dataFreshness:{price:report.closeDate||poc.dataDate,institution:capital.latestDate,tdcc:holder.latestDate,revenue:operating.latestRevenueMonth,financial:operating.latestQuarter,poc:poc.dataDate},originalExecutionAllowed:report.shitoOriginalExecutionAllowed,sourceNote:"只使用官方公開資料與原本盤後日K；沒有穩定官方來源的欄位不顯示，也不計分。"};
}
...
function renderShitoAssistantV47(e){
  ...
  root.querySelector("#saGateGrid").innerHTML=gateHtml("第一重｜營運把關",e.operatingGateEvidence)+gateHtml("第二重｜大戶把關",e.holderGateEvidence)+gateHtml("第三重｜資金把關",e.capitalGateEvidence);
  const p=e.anchoredVolumeProfileEvidence;
  root.querySelector("#saPoc").innerHTML=p.status==="pass"?`<div class="sa-price">${range(p.pocLower,p.pocUpper)}</div><p><strong>現在怎麼看：</strong>${p.pocRole}</p><p><strong>拉回狀態：</strong>${p.pocRetestState}</p><p>最近成交密集區：${p.nearestHvnZone?range(p.nearestHvnZone.lower,p.nearestHvnZone.upper):"尚未辨識"}<br>最近成交較少區：${p.nearestLvnZone?range(p.nearestLvnZone.lower,p.nearestLvnZone.upper):"尚未辨識"}</p><small>${p.anchorStartDate}～${p.anchorEndDate}｜用價格區間估算，不是逐筆成交分布</small>`:`<p>${STATUS_ICON[p.status]} ${p.label}</p>`;
...
function buildAssistantText(e){
  ...
  return ["","━━━━━━━━━━━━━━━━━━","🛡️ 少爺助理｜三重選股把關＋區段量價防守","━━━━━━━━━━━━━━━━━━",`三重把關：${e.summary}`,`營運把關：${STATUS_LABEL[o.status]}`,`大戶把關：${STATUS_LABEL[h.status]}（只看集保資料，不把大戶直接叫做內資）`,`資金把關：${STATUS_LABEL[c.status]}`,"",`主要成交密集區：${p.status==="pass"?range(p.pocLower,p.pocUpper):p.label}`,`目前角色：${p.pocRole||"資料不足"}`,`拉回狀態：${p.pocRetestState||"資料不足"}`,`短線與突破檢查：${e.momentumBreakoutEvidence.summary}`,`主要加分證據：${e.strengths.join("、")||"目前資料不足"}`,`主要風險證據：${e.risks.join("、")||"目前沒有新增明確警訊"}`,`資料日期：${document.getElementById("saDates")?.textContent||"各欄位依上方標示"}`,"",`結論：${document.getElementById("saConclusion")?.textContent||"本模組只作輔助證據。"}`,"提醒：這是盤後資料，不是即時看盤；本模組屬輔助證據，仍須通過原有進場檢查。"].join("\n");
}
...
function appendAssistantCanvasV47(base,e,industry=null){
  if(!e)return base;const extra=760,out=document.createElement("canvas");out.width=base.width;out.height=base.height+extra;const c=out.getContext("2d");c.drawImage(base,0,0);c.fillStyle="#f4f7fb";c.fillRect(0,base.height,out.width,extra);c.fillStyle="#10243d";c.fillRect(0,base.height,out.width,76);c.fillStyle="#fff";c.font="900 28px sans-serif";c.fillText("少爺助理｜三重選股把關＋區段量價防守",42,base.height+48);
  const cards=[["營運把關",e.operatingGateEvidence.status],["大戶把關",e.holderGateEvidence.status],["資金把關",e.capitalGateEvidence.status]],y=base.height+98,w=(out.width-108)/3;cards.forEach((a,i)=>{const x=36+i*(w+18);c.fillStyle="#fff";c.strokeStyle="#cfd8e5";c.lineWidth=2;c.beginPath();c.roundRect(x,y,w,104,14);c.fill();c.stroke();c.fillStyle=a[1]==="pass"?"#137a48":a[1]==="fail"?"#9b2f3b":a[1]==="near"?"#a85f08":"#677386";c.font="900 22px sans-serif";c.fillText(`${STATUS_ICON[a[1]]} ${a[0]}`,x+18,y+35);c.font="800 18px sans-serif";c.fillText(STATUS_LABEL[a[1]],x+18,y+72);});
  const p=e.anchoredVolumeProfileEvidence;c.fillStyle="#fff";c.strokeStyle="#cfd8e5";c.beginPath();c.roundRect(36,y+126,out.width-72,164,14);c.fill();c.stroke();c.fillStyle="#153a67";c.font="900 23px sans-serif";c.fillText(`主要成交密集區（估算）：${p.status==="pass"?range(p.pocLower,p.pocUpper):p.label}`,56,y+162);c.fillStyle="#34445a";c.font="700 18px sans-serif";wrapCanvasText(c,`${p.pocRole||""}｜${p.pocRetestState||""}`,56,y+199,out.width-112,26,2);c.font="700 16px sans-serif";c.fillStyle="#657286";wrapCanvasText(c,`資料：${p.anchorStartDate||"-"}～${p.anchorEndDate||"-"}；用48個價格區估算，不是逐筆成交分布。`,56,y+255,out.width-112,24,2);
  c.fillStyle="#fff8e8";c.strokeStyle="#e0b86d";c.beginPath();c.roundRect(36,y+310,out.width-72,150,14);c.fill();c.stroke();c.fillStyle="#794807";c.font="900 21px sans-serif";c.fillText(`主要優勢：${e.strengths.join("、")||"目前資料不足"}`,56,y+347);c.fillStyle="#86303a";c.fillText(`主要風險：${e.risks.join("、")||"目前沒有新增明確警訊"}`,56,y+386);c.fillStyle="#34445a";c.font="750 17px sans-serif";wrapCanvasText(c,e.originalExecutionAllowed?"新增證據只作輔助；原有進場檢查目前允許執行，仍須遵守原本風險界線。":"新增證據不能改寫原有進場檢查；目前不把證據分數當成直接買進訊號。",56,y+425,out.width-112,24,2);
  c.fillStyle="#eef7f3";c.strokeStyle="#9dc9b5";c.beginPath();c.roundRect(36,y+478,out.width-72,116,14);c.fill();c.stroke();c.fillStyle="#174f43";c.font="900 20px sans-serif";c.fillText(`同產業有沒有一起轉強：${industry?.displayIndustry||"資料不足"}｜${industry?.industryState||"產業觀察"}`,56,y+513);c.fillStyle="#34445a";c.font="750 17px sans-serif";wrapCanvasText(c,industry?.mainWaveCount==null?"尚未同時完成兩種選股，暫不硬說已形成產業共振。":"同產業主升段 "+industry.mainWaveCount+" 檔｜當沖 "+industry.dayTradeCount+" 檔｜雙榜同股 "+industry.overlapCount+" 檔",56,y+546,out.width-112,24,2);c.fillStyle="#657286";c.font="700 15px sans-serif";c.fillText("產業背景只作輔助，不會改變原有進場檢查。",56,y+581);
  c.fillStyle="#5f6e82";c.font="700 15px sans-serif";wrapCanvasText(c,`盤後資料｜股價 ${safeDate(e.dataFreshness.price)}｜法人 ${safeDate(e.dataFreshness.institution)}｜集保 ${safeDate(e.dataFreshness.tdcc)}｜本模組屬輔助證據，仍須通過原有進場檢查。`,44,out.height-54,out.width-88,22,2);out.dataset.reportMode=base.dataset.reportMode||"";out.dataset.language=base.dataset.language||"plain-zh-TW";out.dataset.snrAudit=base.dataset.snrAudit||"";out.dataset.layoutAudit=base.dataset.layoutAudit||"";return out;
}
...
window.ShitoAssistantV47={rules:SHITO_ASSISTANT_RULES_V1,anchoredVolumeProfileEvidence,operatingGateEvidence,holderGateEvidence,capitalGateEvidence,momentumBreakoutEvidence,buildShitoAssistantEvidence,summaryText};
})();
