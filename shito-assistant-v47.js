...
const SHITO_ASSISTANT_RULES_V1=Object.freeze({
  version:"石頭少爺 Agent V47 正式版｜R5.3.2.4.11 主升段完整輸出與候選分流修正版",
  profileBins:48,
  maxDailyBars:240,
  minDailyBars:20,
...
function appendAssistantCanvasV47(base,e,industry=null){
  if(!e)return base;const extra=760,out=document.createElement("canvas");out.width=base.width;out.height=base.height+extra;const c=out.getContext("2d");c.drawImage(base,0,0);c.fillStyle="#f4f7fb";c.fillRect(0,base.height,out.width,extra);c.fillStyle="#10243d";c.fillRect(0,base.height,out.width,76);c.fillStyle="#fff";c.font="900 28px sans-serif";c.fillText("少爺助理｜公司、大戶、法人＋主要成交價區",42,base.height+48);
  const cards=[["公司營運",e.operatingGateEvidence.status],["大戶持股",e.holderGateEvidence.status],["法人買盤",e.capitalGateEvidence.status]],y=base.height+98,w=(out.width-108)/3;cards.forEach((a,i)=>{const x=36+i*(w+18);c.fillStyle="#fff";c.strokeStyle="#cfd8e5";c.lineWidth=2;c.beginPath();c.roundRect(x,y,w,104,14);c.fill();c.stroke();c.fillStyle=a[1]==="pass"?"#137a48":a[1]==="fail"?"#9b2f3b":a[1]==="near"?"#a85f08":"#677386";c.font="900 22px sans-serif";c.fillText(`${STATUS_ICON[a[1]]} ${a[0]}`,x+18,y+35);c.font="800 18px sans-serif";c.fillText(STATUS_LABEL[a[1]],x+18,y+72);});
