...
function appendAssistantCanvasV47(base,e,industry=null){
  // R4.5：只調整個股圖片展示。網頁、文字報告與底層大戶／產業資料全部保留。
  // 指定移除「大戶持股」卡與「同產業有沒有一起轉強」區塊後，縮回實際畫布高度，不留空洞。
  if(!e)return base;const extra=620,out=document.createElement("canvas");out.width=base.width;out.height=base.height+extra;const c=out.getContext("2d");c.drawImage(base,0,0);c.fillStyle="#f4f7fb";c.fillRect(0,base.height,out.width,extra);c.fillStyle="#10243d";c.fillRect(0,base.height,out.width,76);c.fillStyle="#fff";c.font="900 28px sans-serif";c.fillText("少爺助理｜公司、法人＋主要成交價區",42,base.height+48);
  const cards=[["公司營運",e.operatingGateEvidence.status],["法人買盤",e.capitalGateEvidence.status]],y=base.height+98,w=(out.width-90)/2;cards.forEach((a,i)=>{const x=36+i*(w+18);c.fillStyle="#fff";c.strokeStyle="#cfd8e5";c.lineWidth=2;c.beginPath();c.roundRect(x,y,w,104,14);c.fill();c.stroke();c.fillStyle=a[1]==="pass"?"#137a48":a[1]==="fail"?"#9b2f3b":a[1]==="near"?"#a85f08":"#677386";c.font="900 22px sans-serif";c.fillText(`${STATUS_ICON[a[1]]} ${a[0]}`,x+18,y+35);c.font="800 18px sans-serif";c.fillText(STATUS_LABEL[a[1]],x+18,y+72);});
  ...
  const imageStrengths=(e.strengths||[]).filter(text=>!/大戶|集保|小額股東/.test(text));
  const imageRisks=(e.risks||[]).filter(text=>!/大戶|集保|小額股東/.test(text));
  c.fillStyle="#fff8e8";c.strokeStyle="#e0b86d";c.beginPath();c.roundRect(36,y+310,out.width-72,150,14);c.fill();c.stroke();c.fillStyle="#794807";c.font="900 21px sans-serif";c.fillText(`主要優勢：${imageStrengths.join("、")||"目前資料不足"}`,56,y+347);c.fillStyle="#86303a";c.fillText(`主要風險：${imageRisks.join("、")||"目前沒有新增明確警訊"}`,56,y+386);c.fillStyle="#34445a";c.font="750 17px sans-serif";wrapCanvasText(c,e.originalExecutionAllowed?"新增證據只作輔助；原有進場檢查目前允許執行，仍須遵守原本風險界線。":"新增證據不能改寫原有進場檢查；目前不把證據分數當成直接買進訊號。",56,y+425,out.width-112,24,2);
  c.fillStyle="#5f6e82";c.font="700 15px sans-serif";wrapCanvasText(c,`盤後資料｜股價 ${safeDate(e.dataFreshness.price)}｜法人 ${safeDate(e.dataFreshness.institution)}｜本模組屬輔助證據，仍須通過原有進場檢查。`,44,out.height-54,out.width-88,22,2);out.dataset.reportMode=base.dataset.reportMode||"";out.dataset.language=base.dataset.language||"plain-zh-TW";out.dataset.snrAudit=base.dataset.snrAudit||"";out.dataset.layoutAudit=base.dataset.layoutAudit||"";out.dataset.r45ImageBlocksRemoved="holder-card,industry-panel";return out;
}
