...
const SHITO_ASSISTANT_RULES_V1=Object.freeze({
  version:"石頭少爺 Agent V47 正式版｜R5.3.2.4.7 末升段與突破成熟度風險層強化版",
  profileBins:48,
  maxDailyBars:240,
  minDailyBars:20,
...
function anchoredVolumeProfileEvidence(report){
  ...
  return {status:"pass",label:"已用盤後日K完成區段估算",dataTiming:"POST_CLOSE",moneyFlowRole:"SECONDARY_CONFIRMATION",anchorType:anchor.type,anchorStartDate:rows[0].date,anchorEndDate:rows.at(-1).date,pocPrice:(poc.lower+poc.upper)/2,pocLower:poc.lower,pocUpper:poc.upper,pocVolumeShare:total?poc.volume/total*100:null,nearestHvnZone:nearH,nearestLvnZone:nearL,currentPriceVsPoc:close>poc.upper?"above":close<poc.lower?"below":"inside",pocRole:role,pocRetestState:held?(confirmations>=3&&executionAllowed?"拉回後守穩，輔助證據足夠":"拉回有守，但原有進場檢查尚未允許，不能當買進訊號"):touched?"最近碰到此區，尚未完成守穩確認":"最近10個交易日尚未回測此區",supportConfirmations:confirmations,volumeProfileDataQuality:`有效日K ${rows.length} 根`,calculationMethod:"價格分成48箱；每根日K以（最高＋最低＋收盤）÷3歸入一箱並累加成交量，屬區段估算，不是逐筆成交分布",dataDate:rows.at(-1).date,rowsUsed:rows.length,boxWidth:width};
}
...
function buildShitoAssistantEvidence(report,raw){
  ...
  return {version:"石頭少爺 Agent V47 正式版｜R5.3.2.4.7 末升段與突破成熟度風險層強化版",dataTiming:"POST_CLOSE",moneyFlowRole:"SECONDARY_CONFIRMATION",executionPolicy:"EVIDENCE_ONLY_NEVER_OVERRIDES_EXECUTION_ALLOWED",operatingGateEvidence:operating,holderGateEvidence:holder,capitalGateEvidence:capital,anchoredVolumeProfileEvidence:poc,momentumBreakoutEvidence:momentum,summary:`通過${main.pass}／${main.total}重${main.missing?`｜另有${main.missing}重資料不足`:""}`,strengths:strengths.slice(0,2),risks:risks.slice(0,2),dataFreshness:{price:report.closeDate||poc.dataDate,institution:capital.latestDate,tdcc:holder.latestDate,revenue:operating.latestRevenueMonth,financial:operating.latestQuarter,poc:poc.dataDate},originalExecutionAllowed:report.shitoOriginalExecutionAllowed,sourceNote:"只使用官方公開資料與原本盤後日K；沒有穩定官方來源的欄位不顯示，也不計分。"};
}
