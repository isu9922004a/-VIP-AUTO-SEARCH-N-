/* 石頭少爺 Agent V47 R5.3.2｜產業輪動與雙榜共振證據層 */
(function(){
"use strict";
...
function buildIndustryRotation(mainCandidates=[],dayCandidates=[],resources){
  ...
  return {schema:SHITO_INDUSTRY_CONTEXT_SCHEMA_V1,dataDate:resources.snapshot?.dataDate||null,industries:results,byIndustry,industrySelectionConsensus,mainHot:results.filter(x=>x.mainWaveCount).sort((a,b)=>b.mainWaveCount-a.mainWaveCount||b.selectionRatio-a.selectionRatio).slice(0,6),dayHot:results.filter(x=>x.dayTradeCount).sort((a,b)=>b.dayTradeCount-a.dayTradeCount||b.selectionRatio-a.selectionRatio).slice(0,6)};
}
...
function industryText(rotation,type){if(!rotation)return "";const rows=type==="main"?rotation.mainHot:rotation.dayHot;return ["","━━━━━━━━━━━━━━━━━━","【少爺助理｜同產業有沒有一起轉強】","━━━━━━━━━━━━━━━━━━",`${type==="main"?"主升段":"當沖"}熱門產業：${rows.map(x=>`${x.industryName} ${type==="main"?x.mainWaveCount:x.dayTradeCount}檔（${fmt((x.selectionRatio||0)*100)}%）`).join("｜")||"尚無"}`,`雙榜產業共振：${rotation.industrySelectionConsensus.map(x=>`${x.industryName} 主升${x.mainWaveCount}／當沖${x.dayTradeCount}／重疊${x.overlapCount}`).join("｜")||"尚未形成"}`,`產業資料日：${rotation.dataDate||"資料不足"}`,"提醒：產業背景只作輔助，不會改變原榜排名、原分數或原有進場檢查。"].join("\n");}
function stockIndustryText(x){if(!x)return "";return ["","━━━━━━━━━━━━━━━━━━","【少爺助理｜同產業有沒有一起轉強】","━━━━━━━━━━━━━━━━━━",`產業：${x.displayIndustry}`,`官方產業：${x.industryName}`,`次產業：${x.subIndustryName||"沒有可信對照，不硬猜"}`,`產業狀態：${x.industryState}`,`主升段同產業：${x.mainWaveCount??"資料不足"}｜當沖同產業：${x.dayTradeCount??"資料不足"}｜雙榜同股：${x.overlapCount??"資料不足"}`,"產業背景只作輔助，實際進場仍須通過原有進場檢查。"].join("\n");}

function appendIndustryCanvasV47(base,rotation,type){
  if(!base||!rotation)return base;
  const rows=(type==="day"?rotation.dayHot:rotation.mainHot).slice(0,3),extra=165,out=document.createElement("canvas");out.width=base.width;out.height=base.height+extra;const c=out.getContext("2d");c.drawImage(base,0,0);c.fillStyle="#eef7f3";c.fillRect(0,base.height,out.width,extra);c.fillStyle="#174f43";c.font="900 22px sans-serif";c.fillText("少爺助理｜同產業有沒有一起轉強",42,base.height+42);c.fillStyle="#34445a";c.font="750 17px sans-serif";c.fillText(`${type==="day"?"短線":"主升段"}熱門產業：${rows.map(x=>`${x.industryName} ${type==="day"?x.dayTradeCount:x.mainWaveCount}檔`).join("｜")||"尚無"}`,42,base.height+80);c.fillText(`雙榜產業共振：${rotation.industrySelectionConsensus.slice(0,3).map(x=>`${x.industryName}（重疊${x.overlapCount}）`).join("｜")||"尚未形成"}`,42,base.height+112);c.fillStyle="#657286";c.font="700 15px sans-serif";c.fillText(`產業資料日：${rotation.dataDate||"資料不足"}｜盤後整理，不是即時看盤；不改原分數、排名與進場檢查。`,42,base.height+145);
...
window.ShitoIndustryContextV47={schemas:{industry:SHITO_INDUSTRY_CONTEXT_SCHEMA_V1},mappedMember,buildIndustryRotation,loadIndustryContextForStock};
})();
